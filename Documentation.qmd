...
Together with dates, Ther has also two variables concerning the duration of the pharmacological therapy (one with the unit, and one with the number). We decided to convert everything to days (if the unit was NA we lost the information, since in this case we cannot exclude that other measure units were used). We made so that a therapy started and ended the same day has a duration in days of 1, and one ended the day after the start has a duration of 2, and so on. In parallel with standardizing the duration provided by the FDA, we also calculated it as the difference between the end date and the start date + 1. This way we found that for only the 8% of the rows of Ther a duration was provided by the FDA, while we managed to calculate it for ≈32% of the rows. In 1.7% of the total rows there were some differences in the duration calculated and the duration provided (0.7% because the duration provided by the FDA started from 0 days instead of 1). Most of the other differences were due to the fact that we rounded to the day. We decided to keep the calculated duration when available, and when not to get the duration provided by the FDA (filling +1.5%).

Finally, we used the duration to impute start_dt or end_dt when possible.
